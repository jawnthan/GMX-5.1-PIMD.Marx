...
/* MOD 1 : simplify "if" from old adress */
/* MOD 2 : comment "if wp<1 & wp>0" out */
/* MOD 3 : comment "if" out */

        if ((!bCG) && weight_cg1 < ALMOST_ZERO)
        {
            continue;
        }
        if ((bCG) && weight_cg1 >= ALMOST_ONE)
        {
            continue;
        }
        ...
        for (k = nj0; (k < nj1); k++)
        {
         ...
         if ((!bCG) && weight_cg2 < ALMOST_ZERO)
         {
             continue;
         }
         if ((bCG) && weight_cg2 >= ALMOST_ONE)
         {
             continue;
         }
/* MOD 2 
         if ((weight_product > ALMOST_ZERO) && (weight_product < ALMOST_ONE))
         {   
*/
